...
async function getCartItems(conn, userId) {
  const [rows] = await conn.query(
    `SELECT ci.product_id, ci.quantity, ci.unit_price, (ci.quantity * ci.unit_price) AS subtotal,
     ...
     WHERE ci.user_id = ? FOR UPDATE`, [userId]
  );
  return rows;
}
...
router.post('/checkout', ensureAuthenticated,
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      req.session.flash = { type: 'error', message: 'Data checkout tidak valid' };
    ...
    try {
      ...
      for (const it of items) {
        if (it.quantity > it.stock) {
          await conn.rollback();
          req.session.flash = { type: 'error', message: `Stok tidak cukup untuk ${it.name}` };
          return res.redirect('/cart');
        }
      }

      // Create order
      const { address, city, postal_code, country, phone, payment_method } = req.body;
      const [orderRes] = await conn.query(
        ...
        [userId, totals.total, payment_method, address, city, postal_code, country, phone]
      );
      ...
      for (const it of items) {
        ...
        const [upd] = await conn.query('UPDATE products SET stock = stock - ? WHERE id = ? AND stock >= ?', [it.quantity, it.product_id, it.quantity]);
        if (upd.affectedRows === 0) {
          await conn.rollback();
          req.session.flash = { type: 'error', message: `Stok berubah untuk ${it.name}, silakan coba lagi` };
          return res.redirect('/cart');
        }
      }
      ...
      await conn.commit();
      req.session.flash = { type: 'success', message: `Pesanan berhasil dibuat (#${orderId}). Pembayaran: COD` };
      res.redirect('/');
    } catch (e) {
      console.error(e);
      await conn.rollback();
      req.session.flash = { type: 'error', message: 'Terjadi kesalahan saat checkout' };
