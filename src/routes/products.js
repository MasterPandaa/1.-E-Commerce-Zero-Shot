...
router.get("/", async (req, res) => {
  const [products] = await pool.query(
    "SELECT id, name, price, image_url FROM products WHERE is_active = 1 ORDER BY created_at DESC LIMIT 12",
  );
...
router.get("/products", async (req, res) => {
  ...
  const [rows] = await pool.query(
    `SELECT SQL_CALC_FOUND_ROWS id, name, price, image_url FROM products ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
...
router.post(
  ...
  async (req, res) => {
    ...
    await pool.query("UPDATE products SET is_active = 0 WHERE id = ? LIMIT 1", [
      req.params.id,
    ]);
    res.redirect("/admin/products");
  },
