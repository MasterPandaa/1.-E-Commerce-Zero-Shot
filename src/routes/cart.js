...
router.post(
  ...
  async (req, res) => {
    ...
    const [prodRows] = await pool.query(
      "SELECT id, price, stock, is_active FROM products WHERE id = ? LIMIT 1",
      [product_id],
    );
    ...
    const newQty = Math.max(
      1,
      Math.min(product.stock, currentQty + Number(quantity)),
    );
    if (existRows.length) {
      await pool.query(
        "UPDATE cart_items SET quantity = ?, unit_price = ? WHERE user_id = ? AND product_id = ? LIMIT 1",
        [newQty, product.price, userId, product.id],
      );
    } else {
      await pool.query(
        "INSERT INTO cart_items (user_id, product_id, quantity, unit_price) VALUES (?, ?, ?, ?)",
        [userId, product.id, newQty, product.price],
      );
    }
    req.session.flash = {
      type: "success",
...
router.post(
  ...
  async (req, res) => {
    ...
    if (Number(quantity) === 0) {
      await pool.query(
        "DELETE FROM cart_items WHERE user_id = ? AND product_id = ? LIMIT 1",
        [userId, product_id],
      );
      return res.redirect("/cart");
    }
    const [prodRows] = await pool.query(
      "SELECT stock, price FROM products WHERE id = ? LIMIT 1",
      [product_id],
    );
    ...
    await pool.query(
      "UPDATE cart_items SET quantity = ?, unit_price = ? WHERE user_id = ? AND product_id = ? LIMIT 1",
      [qty, prodRows[0].price, userId, product_id],
    );
    res.redirect("/cart");
  },
);
