...
router.post('/login',
  ...
  async (req, res) => {
    ...
    if (rows.length === 0) {
      return res.status(400).render('auth/login', { title: 'Login', errors: [{ msg: 'Email atau password salah' }] });
    }
    const user = rows[0];
...
router.post('/register',
  ...
  async (req, res) => {
    ...
    if (exist.length > 0) {
      return res.status(400).render('auth/register', { title: 'Register', errors: [{ msg: 'Email sudah terdaftar' }] });
    }
...
router.post('/forgot', body('email').isEmail().normalizeEmail(), async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).render('auth/forgot', { title: 'Lupa Password', errors: errors.array() });
  }
  const { email } = req.body;
  const [rows] = await pool.query('SELECT id FROM users WHERE email = ? LIMIT 1', [email]);
  if (rows.length === 0) {
    // Avoid leaking info
    return res.render('auth/forgot', { title: 'Lupa Password', info: 'Jika email terdaftar, tautan reset telah dikirim.' });
  }
  const userId = rows[0].id;
  const token = crypto.randomBytes(32).toString('hex');
  const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
  const expiresAt = new Date(Date.now() + 60 * 60 * 1000); // 1 hour
  await pool.query('INSERT INTO password_resets (user_id, token_hash, expires_at) VALUES (?, ?, ?)', [userId, tokenHash, expiresAt]);
...
router.get('/reset/:token', ensureGuest, async (req, res) => {
  ...
  const [rows] = await pool.query('SELECT id, user_id, expires_at, used FROM password_resets WHERE token_hash = ? LIMIT 1', [tokenHash]);
...
router.post('/reset/:token',
  ...
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).render('auth/reset', { title: 'Reset Password', token: req.params.token, errors: errors.array() });
    }
